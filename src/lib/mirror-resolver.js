...
// Resolve a base64 mirror payload into a raw playable URL.
// Returns { url, player } where player is 'direct' | 'hls' | 'embed'.
// The returned url is the raw upstream URL (no /api/media wrapping).
export async function resolveMirrorPayload(payload) {
  let obj;
  try {
    obj = JSON.parse(Buffer.from(payload, 'base64').toString('utf-8'));
  } catch {
    throw new Error('Payload mirror tidak valid.');
  }
  ...
  if (!src) {
    throw new Error('Tidak ada source video untuk mirror ini.');
  }

  const resolvedSrc = src.startsWith('//') ? `https:${src}` : src;
  const host = new URL(resolvedSrc).hostname.toLowerCase();

  if (host === 'filedon.co' || host.endsWith('.filedon.co')) {
    const url = await extractFiledonUrl(resolvedSrc);
    if (!url) throw new Error('URL video Filedon tidak ditemukan.');
    return { url, player: 'direct' };
  }

  if (host === 'odvidhide.com' || host.endsWith('.odvidhide.com')) {
    const url = await extractVidhideUrl(resolvedSrc);
    if (!url) throw new Error('URL video VidHide tidak ditemukan.');
    return { url, player: 'hls' };
  }

  if (
    host === 'desustream.net' || host.endsWith('.desustream.net')
    ...
    if (!url) throw new Error('URL video DesuStream tidak ditemukan.');
    return { url, player: 'direct' };
  }

  return { url: resolvedSrc, player: 'embed' };
}
